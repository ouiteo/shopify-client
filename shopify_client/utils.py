...
def create_hmac(
    ...
    if build_query:
        ...
        data = join_key.join(query_string).encode(encoding)

    # Generate the HMAC value
    hmac_local = hmac.new(secret.encode(encoding), data, hashlib.sha256)
    if encode:
        # For webhooks
        return base64.b64encode(hmac_local.digest())
    # For 0Auth and proxy
    return hmac_local.hexdigest().encode(encoding)
...
def hmac_verify(source: str, secret: str, params: dict, hmac_header: Optional[str] = None) -> bool:
    ...
    if source == "standard":
        ...
        hmac_param = params["hmac"].encode(encoding)
        ...
        hmac_param = params["signature"].encode(encoding)
        ...
        # Webhook data method
        hmac_param = hmac_header.encode(encoding)
        kwargs = {
            "data": params.encode(encoding),
            "raw": True,
            "encode": True,
        }
